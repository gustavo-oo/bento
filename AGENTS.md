...
CLI that installs/updates/removes, in a consumer project, the personal opencode workflow: the `small-prs` skill (PR limits + sessions/live stack), the `self-review` skill (internal review gate with 2 agents), the ponytail plugin, 14 vendored superpowers skills with scoped agents (`flash`, `explorer`, `verify`, `reviewer`, `browser`), MCP servers (codegraph, agent-browser), the agent-browser/taste-skill skills, the direct output style (`i-have-adhd` skill + always-on `instructions`), and a stack-aware pre-push hook.
...
- `install` targets a consumer project and requires `gh` (installs the gh-stack extension) → exit 1 without `gh`; `update` and `uninstall` do NOT touch gh-stack.
- `install`/`update` copy `lib/`, `bin/`, `templates/` into `.bento/` (+ `.bento/VERSION`), vendor the 14 superpowers skills, create the scoped agents (`flash`, `explorer`, `verify`, `reviewer`, `browser`) in `.opencode/agents/` and the `default_agent` (only if absent and the on-disk `flash` is from bento), remove the superpowers plugin from older installs, and create bento's skills (`.opencode/skills/small-prs`, `self-review`, `taste-skill`, and, unless flagged, `agent-browser`), the output style (`.opencode/skills/i-have-adhd`, `.opencode/instructions/i-have-adhd.md`, and the `instructions` key in `opencode.json`/`.jsonc`; skip with `--no-output-style`), the `scripts/pr-split-verify.mjs` shim, `.bento.yaml` (single config: PR limits + artifact language; only if absent, and a legacy `.pr-limits.yaml` is merged into it and removed), and the `## Bento (small-prs)` section in AGENTS.md. On install/update, obsolete agents from older installs (`superpowers`, `orchestrator`, `implementer`) are only warned about, never removed.
- install/update flags: `--no-agents`, `--no-superpowers` (skips vendored skills; does not touch the plugin or advance the `.bento/skills` snapshot), `--no-profile` (skips profile agents + `default_agent`), `--no-ponytail`, `--no-hooks`, `--no-shim` (skips the `scripts/pr-split-verify.mjs` shim), `--no-codegraph`, `--no-agent-browser`, `--no-output-style`, and `--language "<value>"` (artifact language in `.bento.yaml`; on `install` a TTY prompts for it when the file is missing; `update` never prompts).
...
- `lib/tools.mjs`: installs global CLIs (`@colbymchenry/codegraph`, `agent-browser`); failures warn instead of erroring. Global tools (CLIs and gh-stack) are shared across consumers and are NEVER removed by `uninstall`; a CLI regression test pins this by putting fake `npm`/`gh` on PATH and asserting they are not called.
...
- Dogfooding: this repo consumes bento via `npm run dogfood:setup`/`npm run dogfood` (`--no-shim`); `.bento/`, `.opencode/`, `opencode.json[c]`, and `.codegraph/` are git-ignored; the committed `scripts/pr-split-verify.mjs` shim points at `../lib/validate.mjs` (live).
...
- `templates/agents/`: source of the `flash` (single primary), `explorer`, `verify`, `reviewer`, and `browser` agents.
