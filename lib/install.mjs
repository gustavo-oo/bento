...
if (!noAgentBrowser) {
  ...
  if (!noAgents) {
    ...
    if (!/^## Bento\b/m.test(content)) {
      ...
      console.error('aviso: seção ## Bento existente não cita o self-review; atualize o AGENTS.md à mão para receber o gate.');
    }
  }
  ...
  if (existsSync(flashPath) && isBentoAgent(flashPath) && !readFileSync(flashPath, 'utf8').includes('self-review')) {
    console.error('aviso: flash.md não cita o gate self-review (instalação anterior); edite-o ou remova-o e rode update de novo.');
  }
